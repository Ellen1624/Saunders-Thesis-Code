...
xlabel('Longitude');
ylabel('Latitude');
title('Holey Velocity Field with a Missing Patch');
axis equal;
grid on;
...
n = 0.005; % gradient descent step size
...
%print('-dpdf','S_NN_ROMS1_15_grad_err.pdf');

%%

% 3D plotting the original components - not that useful of a visualization

% original
%{
% original u components
figure;
surf(ln_save, lt_save, u_save);
xlabel('Longitude');
ylabel('Latitude');
zlabel('u\_save');
title('3D Plot of u componenets over Latitude and Longitude');
shading interp;  % Optional: makes the surface smoother
colorbar;        % Optional: adds a color scale bar

% original v components
figure;
surf(ln_save, lt_save, v_save);
xlabel('Longitude');
ylabel('Latitude');
zlabel('v\_save');
title('3D Plot of v componenets over Latitude and Longitude');
shading interp;  % Optional: makes the surface smoother
colorbar;        % Optional: adds a color scale bar
%}
...
Accuracy = sum((u_save(:) - u_new(:)).^2 + (v_save(:) - v_new(:)).^2)  % component-wise MSE between the original and imputed field

%%

% imputed components
%{
% imputed u components
figure;
surf(ln_save, lt_save, u_new);
xlabel('Longitude');
ylabel('Latitude');
zlabel('u\_save');
title('Imputed u Componenets');
shading interp;  % Optional: makes the surface smoother
colorbar;        % Optional: adds a color scale bar

% imputed v components
figure;
surf(ln_save, lt_save, v_new);
xlabel('Longitude');
ylabel('Latitude');
zlabel('v\_save');
title('Imputed v Components');
shading interp;  % Optional: makes the surface smoother
colorbar;        % Optional: adds a color scale bar
%}

% imputed velocity field
...
set(gcf,'papersize',[4 3],'paperposition',[0 0 4 3]);
print('-dpdf','S_NN_HF_20_imputed.pdf');
